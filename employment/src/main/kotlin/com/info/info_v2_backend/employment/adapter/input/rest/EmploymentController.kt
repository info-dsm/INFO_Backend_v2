...
class EmploymentController(
    ...
    @PostMapping("/generation/class")
    @ResponseStatus(HttpStatus.CREATED)
    fun createGenerationClass(
        @RequestBody request: CreateGenerationClassRequest
    ){
    ...
    fun createGenerationGrade(
        ...
        @RequestParam totalStudent: Int
    ){
        if (Auth.checkIsTeacher()) createGenerationUsecase.createGenerationGrade(generation, totalStudent)
        else throw BusinessException(errorCode = ErrorCode.NO_AUTHORIZATION_ERROR)
    }
